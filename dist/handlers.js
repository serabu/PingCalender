...
*Время указывайте в московском времени (МСК)*
    ...
    bot.onText(/\/add (.+)/, async (msg, match) => {
        ...
        try {
            const id = await (0, database_1.addReminder)(chatId, date, time, message, notifyAt);
    ...
    bot.onText(/\/list/, async (msg) => {
        const chatId = msg.chat.id;
        try {
            const reminders = await (0, database_1.getReminders)(chatId);
    ...
    bot.onText(/\/delete (\d+)/, async (msg, match) => {
        ...
        try {
            const deleted = await (0, database_1.deleteReminder)(id, chatId);
            if (deleted) {
